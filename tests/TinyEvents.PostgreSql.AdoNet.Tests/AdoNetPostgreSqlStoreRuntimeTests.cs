...
public sealed class AdoNetPostgreSqlStoreRuntimeTests
{
    ...
    public AdoNetPostgreSqlStoreRuntimeTests(PostgreSqlFixture fixture)
    {
    ...
    public async Task Competing_workers_claim_message_only_once()
    {
        ...
        var first = store.ClaimPendingAsync(1, "worker-1", now, TimeSpan.FromMinutes(5), CancellationToken.None).AsTask();
        var second = store.ClaimPendingAsync(1, "worker-2", now, TimeSpan.FromMinutes(5), CancellationToken.None).AsTask();
    ...
    [PostgreSqlIntegrationFact]
    public async Task Mark_failed_without_retry_marks_failed_for_owned_message()
    {
        await fixture.ResetSchemaAsync();
        var messageId = Guid.NewGuid();
        ...
        await store.MarkFailedAsync(messageId, "worker-1", "boom", 2, null, CancellationToken.None);

        var row = await ReadFailureAsync(messageId);
        Assert.Equal(TinyOutboxMessageStatus.Failed, row.Status);
    ...
    private TinyPostgreSqlAdoNetOutboxStore NewStore()
    {
        ...
        return new TinyPostgreSqlAdoNetOutboxStore(
            options,
            new TinyPostgreSqlAdoNetWorkerConnectionFactory(options, new EmptyServiceProvider()));
    }
    ...
    private async Task<bool> ExistsAsync(Guid messageId)
    {
        ...
        await connection.OpenAsync();
        await using var command = connection.CreateCommand();
        command.CommandText = """SELECT COUNT(*) FROM "TinyOutbox" WHERE "Id" = @Id;""";
        AddParameter(command, "@Id", messageId);
