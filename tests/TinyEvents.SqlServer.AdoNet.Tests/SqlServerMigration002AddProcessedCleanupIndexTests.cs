...
public sealed class SqlServerMigration002AddProcessedCleanupIndexTests
{
    [Fact]
    public void Migration_002_adds_the_processed_cleanup_index()
    {
        ...
        var migration = SqlServerMigration002AddProcessedCleanupIndex.Create(identity);

        Assert.Equal(2, migration.Version);
        Assert.Equal("002_AddProcessedCleanupIndex", migration.Name);
        Assert.Contains("CREATE INDEX [IX_MyOutbox_ProcessedCleanup]", migration.Sql);
    ...
    public void Packed_default_SQL_asset_matches_migration_002()
    {
        ...
        var migrationSql = SqlServerMigration002AddProcessedCleanupIndex
